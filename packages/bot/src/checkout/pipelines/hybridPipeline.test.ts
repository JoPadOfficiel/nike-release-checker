...
import { runHybridPipeline } from './hybridPipeline.ts'
import type { HybridPipelineOptions } from './hybridPipeline.ts'
...
function makeMockPage(spec: MockSpec = { calls: [] }): Page {
	...
	const evaluateMock = async (_fn: unknown, args?: unknown): Promise<unknown> => {
...
describe('hybridPipeline', () => {
	...
	describe('mapErrorToOutcome', () => {
		it('maps CartViewTimeoutError → timeout outcome', async () => {
			const { mapErrorToOutcome } = await import('../mapErrorToOutcome.ts')
			const { CartViewTimeoutError } = await import('../api/cartViewsApi.ts')
		...
		it('maps NikeCartApiError → error outcome', async () => {
			const { mapErrorToOutcome } = await import('../mapErrorToOutcome.ts')
			const { NikeCartApiError } = await import('../api/cartApi.ts')
	...
	describe('harvestSkuId', () => {
		it('returns skuId from hydration when available', async () => {
			const { harvestSkuId } = await import('../dom/harvestSkuId.ts')
